...
public class NoteActivity extends AppCompatActivity
    ...
    /**
     * saves original note retrieved from mainActivity before any editing
     */
    private void saveOriginalNoteValues() {
        if (mIsNewNote)
            return;
        mOriginalNoteCourseId = mNote.getCourse().getCourseId();
        Log.d(TAG, "saveOriginalNoteValues: courseId " + mOriginalNoteCourseId);
    ...
    protected void onPause() {
        super.onPause();
        if (mIsCancelling) {
            Log.d(TAG, "Cancelling note at position: " + mNoteId);
            if (mIsNewNote) {
    ...
    private void deleteNoteFromDatabase() {
        ...
        AsyncTask task = new AsyncTask() {
            ...
            protected Object doInBackground(Object[] params) {
                int numRows = getContentResolver().delete(mNoteUri, selection, selectionArgs);
    ...
    /**
     * stores the previous note value in the mNote obj using the saved course id
     */
    ...
    private String selectedCourseId() {
        ...
        Log.d(TAG, "selectedCourseId: courseId " + courseId);
        return courseId;
    }
    ...
    /**
     * populates NoteActivity UI widgets with the needed data
     */
    private void displayNote() {
        ...
        CourseEventBroadcastHelper.sendEventBroadcast(this, courseId, "Editing note");   // sends broadcast to subscribers
    }
...
//        mNote = DataManager.getInstance().getNotes().get(mNoteId);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_send_mail) {
            ...
            showReminderNotification();             // shows a notification for a reminder
        }
    ...
    private void showReminderNotification() {
        ...
        // code to use alarmManager to schedule call to broadcast Receiver
        ...
        long currentTimeInMilliseconds = SystemClock.elapsedRealtime();       // gets currentTime
        long ONE_HOUR = 60 * 60 * 1000;                         // one hour in milliseconds, not used here though
        long TEN_SECONDS = 10 * 1000;
        long alarmTime = currentTimeInMilliseconds + TEN_SECONDS;                // time for alarm to go off is 1 hour from previous currentTime
    ...
    private CursorLoader createLoaderCourses() {
        ...
        // cursorLoader init calls the CP.query() method
    ...
    private CursorLoader createLoaderNotes() {
        ...
        // does querying using the CP for a specific note
    ...
    private void loadFinishedNotes(Cursor data) {
        ...
        mNoteCursor.moveToFirst();        // to move cursor to first item in the returned rows
